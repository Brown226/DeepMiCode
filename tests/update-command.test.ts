/** deepmicode update — pure planUpdate + orchestrator with every side effect mocked via test seams. */
...
describe("planUpdate", () => {
  ...
  it("emits npm install -g for npm-installed binaries", () => {
    ...
    expect(plan.command).toEqual(["npm", "install", "-g", "deepmicode@latest"]);
  });
  ...
  it("emits bun add -g for bun-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["bun", "add", "-g", "deepmicode"]);
  ...
  it("emits pnpm add -g for pnpm-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["pnpm", "add", "-g", "deepmicode@latest"]);
  });
  ...
  it("emits yarn global add for yarn-installed binaries", () => {
    ...
    expect(plan.action).toBe("run-install");
    expect(plan.command).toEqual(["yarn", "global", "add", "deepmicode@latest"]);
  });
  ...
  it("returns manual-hint when source cannot be detected — no silent npm fallback", () => {
    ...
    expect(plan.command).toBeUndefined();
    expect(plan.message).toContain("npm install -g deepmicode@latest");
    expect(plan.message).toContain("bun add -g deepmicode");
    expect(plan.message).toContain("pnpm add -g deepmicode@latest");
    expect(plan.message).toContain("yarn global add deepmicode@latest");
  });
});
...
describe("updateCommand", () => {
  ...
  it("spawns npm install -g when npm-installed and behind latest", async () => {
    ...
    expect(h.spawnCalls).toEqual([["npm", "install", "-g", "deepmicode@latest"]]);
    expect(h.exitCode).toBeUndefined();
  });
  ...
  it("spawns bun add -g when bun-installed", async () => {
    ...
    expect(h.spawnCalls).toEqual([["bun", "add", "-g", "deepmicode"]]);
  });
  ...
  it("refuses with manual hint and exits 1 when source cannot be detected", async () => {
    ...
    expect(joined).toContain("npm install -g deepmicode@latest");
    expect(joined).toContain("bun add -g deepmicode");
    expect(h.spawnCalls).toHaveLength(0);
