...
import { type deepmicodeConfig, mcpEnvFor } from "../src/config.js";
...
describe("mcpEnvFor", () => {
  ...
  it("returns undefined when every value is empty (no real overlay to apply)", () => {
    const cfg: deepmicodeConfig = { mcpEnv: { github: { A: "", B: "" } } };
...
describe("buildTransportFromSpec", () => {
  ...
  it("propagates the env overlay to the spawned child (end-to-end)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "deepmicode-mcp-env-"));
    const scriptPath = join(dir, "emit-env.cjs");
    writeFileSync(
      scriptPath,
      'process.stdout.write(JSON.stringify({jsonrpc:"2.0",method:"x",params:{token:process.env.deepmicode_MCP_TEST_TOKEN||""}})+"\\n");',
      "utf8",
    );
    const transport = new StdioTransport({
      command: process.execPath,
      args: [scriptPath],
      env: { deepmicode_MCP_TEST_TOKEN: "from-config-overlay" },
      shell: false,
    });
    try {
