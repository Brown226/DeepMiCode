...
export function checkAuth(
  ...
  const headerToken =
    typeof req.headers["x-deepmicode-token"] === "string"
      ? (req.headers["x-deepmicode-token"] as string)
      : "";
  ...
  if (isMutation) {
    ...
    if (!headerToken || !constantTimeEquals(headerToken, expectedToken)) {
      return {
        ...
        body: JSON.stringify({
          error:
            "mutation requires X-DeepMiCode-Token header (CSRF defence — query token alone is rejected for POST/DELETE).",
        }),
      };
    }
    return null;
