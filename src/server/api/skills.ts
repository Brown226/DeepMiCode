...
function globalSkillsDir(): string {
  return join(homedir(), ".deepmicode", SKILLS_DIRNAME);
}

function projectSkillsDir(rootDir: string): string {
  return join(rootDir, ".deepmicode", SKILLS_DIRNAME);
}
...
export async function handleSkills(
  ...
  if (scope === "project") {
    if (!cwd) {
      return {
        status: 503,
        body: { error: "no active project — open `/dashboard` from `deepmicode code`" },
      };
    }
