 ...
 *  MiMo models require full reasoning_content round-trip in multi-turn thinking mode
 *  (MiMo docs: "多轮对话思考模式下，需完整回传 reasoning_content 字段给接口"),
 *  so we skip stripping when the model is a MiMo model. */
export function stripDroppableReasoningContent(
  messages: ChatMessage[],
  model?: string,
): ReasoningPruneResult {
  // MiMo requires full reasoning_content passthrough — never strip.
