...
export const zhCN: TranslationSchema = {
  ...
  stats: {
    usageHint:
      "运行 `deepmicode chat`、`deepmicode code` 或 `deepmicode run <task>` — 每次对话都会记录",
  ...
  ui: {
    welcome: "随时运行 `deepmicode` 开始聊天 — 您的设置将被记住。",
  ...
  code: {
    workspaceConflict:
      "⚠ 工作区包含另一个智能体平台的文件 ({platforms})。DeepMiCode Code 可能会将其作为项目内容读取；如果不是您想要的，请使用 --dir <your-project> 重新启动。\n",
  ...
  slash: {
    ...
    hooks: {
      description: "列出活跃的 hooks（.deepmicode/ 下的 settings.json）· reload 从磁盘重新读取",
      argsHint: "[reload]",
    },
    permissions: {
      description: "显示 / 编辑 shell 允许列表（内置只读 · 项目级：~/.deepmicode/config.json）",
    ...
    init: {
      description:
        "扫描项目并合成基线 DEEPMICODE.md（模型写入；使用 /apply 审查）。`force` 覆盖已有文件。",
      argsHint: "[force]",
    },
    apply: {
      description:
  ...
  wizard: {
    ...
    reviewLabelTheme: "主题",
    mcpTitle: "DeepMiCode 要为你接入哪些 MCP 服务器？",
  ...
  themePicker: {
    ...
    currentPref: "当前偏好",
    activeNow: "当前生效",
    autoDesc: "使用 DEEPMICODE_THEME 或默认主题",
  },
  planFlow: {
  ...
  errors: {
    ...
    upstream5xxHead:
      "上游服务不可用（{status}），目标地址 {host} — 你配置的 API 端点返回了服务器错误，不是 DeepMiCode 故障。已按指数退避重试 4 次。",
  ...
  handlers: {
    basic: {
      ...
      newInfoSystemReloaded: " · DEEPMICODE.md / 项目记忆已重新加载（下一轮一次性 cache miss）",
      helpTitle: "命令：",
      ...
      helpMemoryPin:
        "  #<note>                  将 <note> 追加到 <project>/DEEPMICODE.md（可提交）。",
      helpMemoryPinEx: "                             示例：#findByEmail 必须区分大小写",
      helpMemoryGlobal:
        "  #g <note>                将 <note> 追加到 ~/.deepmicode/DEEPMICODE.md（全局，不提交）。",
      ...
      helpSessionCustom: "  deepmicode chat --session <name>   使用不同的命名会话",
      ...
      aboutHeader: "DeepMiCode v{version} — 缓存优先的 DeepSeek 编码代理",
    ...
    admin: {
      ...
      updateRetryHint2: "或在另一个终端运行 `deepmicode update` 强制同步执行。",
      ...
      updateUpgradeCmd1: "  deepmicode update           （交互式，支持 --dry-run 预览）",
    ...
    edits: {
      undoCodeOnly: "/undo 仅在 `deepmicode code` 中可用 — 聊天模式不应用编辑。",
      ...
      planCodeOnly: "/plan 仅在 `deepmicode code` 中可用 — 聊天模式不限制工具写入。",
      planOn:
      ...
      checkpointCodeOnly: "/checkpoint 仅在 `deepmicode code` 中可用 — 聊天模式不应用编辑。",
      checkpointNone:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `deepmicode code` 中可用 — 它们编辑项目范围的允许列表（`~/.deepmicode/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession: "未附加会话 — `/plans` 是按会话的。在项目中运行 `deepmicode code` 以获取会话。",
      ...
      replayNoSession:
        "未附加会话 — `/replay` 是按会话的。在项目中运行 `deepmicode code` 以获取会话。",
      replayNoArchives:
    ...
    memory: {
      disabled:
        "记忆已禁用（环境变量 DEEPMICODE_MEMORY=off）。取消设置该变量以重新启用 — 此期间不会固定任何 DEEPMICODE.md 或 ~/.deepmicode/memory 内容。",
      noRoot:
        "此会话无工作目录 — `/memory` 需要一个根目录来解析 DEEPMICODE.md。（在测试环境中运行？）",
      listEmpty:
        "尚无用户记忆。模型可以调用 `remember` 保存一个，或您可以在 ~/.deepmicode/memory/global/ 或项目子目录中手动创建文件。",
    ...
    mcp: {
      noServers:
        '未附加 MCP 服务器。运行 `deepmicode setup` 选择一些，或使用 --mcp "<spec>" 启动。`deepmicode mcp list` 显示目录。注：模型发起的 shell 命令按次审批（allow once / allow always / deny），设计上没有「全局放行」开关。',
      toolsLabel: "  工具     {count}",
    ...
    skill: {
      ...
      listProjectOnly: "  （项目范围仅在 `deepmicode code` 中活跃）",
  ...
  pathConfirm: {
    ...
    allowAlways: "始终允许",
    allowAlwaysDesc: "记住 `{prefix}`，本项目永久允许（写入 ~/.deepmicode/config.json）",
    deny: "拒绝",
  ...
  startup: {
    codeRooted:
      '▸ deepmicode code：根目录 {rootDir}，会话 "{session}" · {tools} 个原生工具{semantic}',
    ephemeral: "（临时）",
    semanticOn: " · 语义搜索已开启",
  },
  doctorErrors: {
  ...
  webErrors: {
    ...
    tavilyMissingKey:
      "web_search: Tavily 后端需要 API 密钥 — 设置 TAVILY_API_KEY 环境变量，或在 ~/.deepmicode/config.json 中配置 `tavilyApiKey`；https://tavily.com 每月 1000 次免费",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity 后端需要 API 密钥 — 设置 PERPLEXITY_API_KEY 环境变量，或在 ~/.deepmicode/config.json 中配置 `perplexityApiKey`；在 https://perplexity.ai/settings/api 获取密钥",
    ...
    exaMissingKey:
      "web_search: Exa 后端需要 API 密钥 — 设置 EXA_API_KEY 环境变量，或在 ~/.deepmicode/config.json 中配置 `exaApiKey`；https://exa.ai 每月 1000 次免费",
    exaUnauthorized:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed: "✓ 已安装 {name} — 重启 `deepmicode code` 以桥接（重载失败：{message}）",
    restartBridge: "✓ 已安装 {name} — 重启 `deepmicode code` 以桥接",
    needsEnv: "  ·  需要环境变量：{env}",
    badgeOfficial: "[官方]",
    badgeSmithery: "[三方]",
    badgeLocal: "[本地]",
  ...
  mcpBrowser: {
    title: "◈ MCP 浏览器",
    empty: "没有挂载 MCP 服务器。运行 `deepmicode setup` 选择一些，或使用 --mcp 启动。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ 运行 `deepmicode setup` 移除此条目，或修复底层问题（缺少 npm 包、网络等）。",
  ...
  mcpCli: {
    ...
    moreAvailable: "还有更多",
    allLoaded: "已全部加载",
    morePagesAvailable: "▸ 还有更多页可用 — `deepmicode mcp list --pages <n>` 或 --all",
    ...
    moreLoaded: "… 还有 {count} 条已加载 — 使用 `deepmicode mcp search <query>` 筛选",
