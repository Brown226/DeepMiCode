...
export const de: TranslationSchema = {
  ...
  stats: {
    ...EN.stats,
    usageHint:
      "Führe `deepmicode chat`, `deepmicode code` oder `deepmicode run <task>` aus – jeden Turn",
    usageDetail: "Hängt eine Zeile an das Log an; `deepmicode stats` fasst sie zusammen.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY ist nicht gesetzt und stdin ist kein TTY (Nachfrage nicht möglich).\n" +
      "Setze die Umgebungsvariable oder starte einmal interaktiv `deepmicode chat`, um einen Schlüssel zu speichern.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "Noch keine gespeicherten Sitzungen – starte `deepmicode chat` (Sitzungen werden automatisch gespeichert, außer mit --no-session).",
  ...
  ui: {
    ...
    welcome:
      "Starte jederzeit `deepmicode`, um zu chatten – deine Einstellungen bleiben gespeichert.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ Arbeitsbereich enthält Dateien einer anderen Agent-Plattform ({platforms}). DeepMiCode Code kann sie als Projektinhalt lesen; starte mit --dir <dein-projekt> neu, falls das nicht gewünscht ist.\n",
  ...
  slash: {
    ...
    permissions: {
      ...
      description:
        "Shell-Allowlist anzeigen / bearbeiten (builtin schreibgeschützt · pro Projekt: ~/.deepmicode/config.json)",
    },
    dashboard: {
      ...EN.slash.dashboard,
      argsHint: "[Stopp]",
    ...
    setup: {
      ...EN.slash.setup,
      description: "Erinnert dich daran, `deepmicode setup` auszuführen",
    },
    semantic: {
      ...EN.slash.semantic,
      description:
    ...
    init: {
      ...EN.slash.init,
      description:
        "Projekt scannen und eine DEEPMICODE.md-Baseline erstellen (Modell schreibt; mit /apply reviewen). `force` überschreibt vorhandene Datei.",
    },
    apply: {
      ...EN.slash.apply,
      description:
  ...
  wizard: {
    ...
    mcpTitle: "Welche MCP-Server soll DeepMiCode für dich einrichten?",
  ...
  errors: {
    ...
    upstream5xxHead:
      "Upstream-Dienst nicht verfügbar ({status}) bei {host} — der konfigurierte API-Endpunkt hat einen Serverfehler zurückgegeben, kein DeepMiCode-Fehler. Bereits 4× mit Backoff wiederholt.",
  ...
  handlers: {
    ...
    basic: {
      ...
      newInfoSystemReloaded:
        " · DEEPMICODE.md / Projekt-Memory neu geladen (nächster Turn zahlt einen Cache-Fehler)",
      helpTitle: "Befehle:",
      ...
      helpMemoryPin:
        "  #<notiz>                  <notiz> an <projekt>/DEEPMICODE.md anhängen (commitierbar).",
      helpMemoryPinEx:
        "                             Beispiel: #findByEmail muss case-insensitive sein",
      helpMemoryGlobal:
        "  #g <notiz>                <notiz> an ~/.deepmicode/DEEPMICODE.md anhängen (global, niemals committed).",
      helpMemoryGlobalEx:
      ...
      helpSessionNone:
        "  deepmicode chat --no-session       Persistenz für diesen Lauf deaktivieren",
      ...
      aboutHeader: "DeepMiCode v{version} — ein Cache-First-DeepSeek-Coding-Agent",
    ...
    admin: {
      ...
      updateRetryHint2:
        "oder führe `deepmicode update` in einem anderen Terminal aus, um es synchron zu erzwingen.",
      updateLatest: "Neueste:  deepmicode {version}",
      updateUpToDate: "Du bist auf dem neuesten Stand. Nichts zu tun.",
      updateNpxHint:
        "Du verwendest npx — der nächste `npx deepmicode ...`-Start lädt automatisch die neueste Version.",
      ...
      updateUpgradeCmd1: "  deepmicode update           (interaktiv, --dry-run wird unterstützt)",
    ...
    edits: {
      ...EN.handlers.edits,
      undoCodeOnly:
        "/undo ist nur innerhalb von `deepmicode code` verfügbar — der Chat-Modus wendet keine Edits an.",
      historyCodeOnly: "/history ist nur innerhalb von `deepmicode code` verfügbar.",
      showCodeOnly: "/show ist nur innerhalb von `deepmicode code` verfügbar.",
      applyCodeOnly:
        "/apply ist nur innerhalb von `deepmicode code` verfügbar (hier gibt es nichts anzuwenden).",
      discardCodeOnly: "/discard ist nur innerhalb von `deepmicode code` verfügbar.",
      planCodeOnly:
        "/plan ist nur innerhalb von `deepmicode code` verfügbar — der Chat-Modus blockiert keine Tool-Schreibzugriffe.",
      planOn:
      ...
      modeCodeOnly: "/mode ist nur innerhalb von `deepmicode code` verfügbar.",
      ...
      commitCodeOnly:
        "/commit ist nur innerhalb von `deepmicode code` verfügbar (benötigt ein Git-Repo als Wurzel).",
      commitUsage:
        'Verwendung: /commit "deine Commit-Nachricht"  — führt `git add -A && git commit -m "…"` in {root} aus',
      walkCodeOnly: "/walk ist nur innerhalb von `deepmicode code` verfügbar.",
      checkpointCodeOnly:
        "/checkpoint ist nur innerhalb von `deepmicode code` verfügbar — der Chat-Modus wendet keine Edits an.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore ist nur innerhalb von `deepmicode code` verfügbar.",
      ...
      cwdCodeOnly: "/cwd ist nur innerhalb von `deepmicode code` verfügbar.",
      cwdUsage:
    ...
    permissions: {
      ...
      mutateCodeOnly:
        "/permissions add / remove / clear sind nur innerhalb von `deepmicode code` verfügbar — sie bearbeiten die projektbezogene Allowlist (`~/.deepmicode/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      ...EN.handlers.plans,
      noSession:
        "Keine Sitzung angehängt — `/plans` ist pro Sitzung. Führe `deepmicode code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      activePlan:
      ...
      replayNoSession:
        "Keine Sitzung angehängt — `/replay` ist pro Sitzung. Führe `deepmicode code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      replayNoArchives:
    ...
    jobs: {
      ...EN.handlers.jobs,
      codeOnly: "/jobs ist nur innerhalb von `deepmicode code` verfügbar.",
      killCodeOnly: "/kill ist nur innerhalb von `deepmicode code` verfügbar.",
      logsCodeOnly: "/logs ist nur innerhalb von `deepmicode code` verfügbar.",
      empty:
    ...
    memory: {
      ...EN.handlers.memory,
      disabled:
        "Memory ist deaktiviert (DEEPMICODE_MEMORY=off in der Umgebung). Entferne die Variable zur Reaktivierung — es werden weder DEEPMICODE.md noch ~/.deepmicode/memory-Inhalte eingefügt.",
      noRoot:
        "Kein Arbeitsverzeichnis in dieser Sitzung — `/memory` benötigt ein Root, um DEEPMICODE.md aufzulösen. (Läuft in einer Test-Umgebung?)",
      listEmpty:
      ...
      layerGlobal:
        "  2. ~/.deepmicode/memory/global/ — dein projektübergreifendes privates Memory.",
      layerProjectHash:
        "  3. ~/.deepmicode/memory/<projekt-hash>/ — privates Memory dieses Projekts.",
    ...
    mcp: {
      ...EN.handlers.mcp,
      noServers:
        'Keine MCP-Server angehängt. Führe `deepmicode setup` aus, um welche auszuwählen, oder starte mit --mcp "<spec>". `deepmicode mcp list` zeigt den Katalog. Hinweis: vom Modell aufgerufene Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen) — kein globales Allow-All-Flag.',
      ...
      catalogHint:
        "Vollständiger Katalog: `deepmicode mcp list` · tiefere Diagnose: `deepmicode mcp inspect <spec>`.",
      fallbackServers: "MCP-Server ({count}):",
      fallbackTools: "Tools im Register ({count}):",
      fallbackChange: "Um diesen Satz zu ändern, beende und führe `deepmicode setup` aus.",
      usageDisableEnable:
    ...
    skill: {
      ...
      listProjectScope:
        "  · <projekt>/.deepmicode/skills/<name>/SKILL.md  (oder <name>.md)  — Projekt-Bereich",
      listGlobalScope:
        "  · ~/.deepmicode/skills/<name>/SKILL.md  (oder <name>.md)  — globaler Bereich",
  ...
  pathConfirm: {
    ...
    allowAlwaysDesc:
      "`{prefix}` für dieses Projekt merken (gespeichert in ~/.deepmicode/config.json)",
    deny: "ablehnen",
  ...
  sessionPicker: {
    ...EN.sessionPicker,
    header: " ◈ DEEPMICODE · Sitzung auswählen ",
  ...
  workspacePicker: {
    ...EN.workspacePicker,
    header: " ◈ DEEPMICODE · Arbeitsbereich auswählen ",
  ...
  modelPicker: {
    ...EN.modelPicker,
    header: " ◈ DEEPMICODE · Einrichtung auswählen ",
  ...
  startup: {
    ...EN.startup,
    codeRooted:
      '▸ deepmicode code: verwurzelt in {rootDir}, Sitzung "{session}" · {tools} native Tool{s}{semantic}',
    ephemeral: "(ephemer)",
  ...
  webErrors: {
    ...
    tavilyMissingKey:
      "web_search: Tavily-Backend benötigt einen API-Schlüssel — setze TAVILY_API_KEY-Umgebungsvariable oder `tavilyApiKey` in ~/.deepmicode/config.json; kostenlose 1000/Monat-Registrierung unter https://tavily.com",
    tavilyUnauthorized:
    ...
    perplexityMissingKey:
      "web_search: Perplexity-Backend benötigt einen API-Schlüssel — setze PERPLEXITY_API_KEY-Umgebungsvariable oder `perplexityApiKey` in ~/.deepmicode/config.json; erhalte einen unter https://perplexity.ai/settings/api",
    ...
    exaMissingKey:
      "web_search: Exa-Backend benötigt einen API-Schlüssel — setze EXA_API_KEY-Umgebungsvariable oder `exaApiKey` in ~/.deepmicode/config.json; kostenlose 1000/Monat-Registrierung unter https://exa.ai",
    exaUnauthorized:
    ...
    braveMissingKey:
      "web_search: Für Brave Search ist ein API-Schlüssel erforderlich — setze die Umgebungsvariable BRAVE_SEARCH_API_KEY (oder BRAVE_API_KEY) oder `braveApiKey` in ~/.deepmicode/config.json; kostenlose Anmeldung mit 2000 Einheiten pro Monat unter https://brave.com/search/api/",
    braveUnauthorized:
  ...
  mcpHealth: {
    ...
    emptyHint:
      "ℹ keine MCP-Server konfiguriert — versuche: `deepmicode setup` zur erneuten Auswahl, oder `deepmicode mcp install filesystem` · Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen), kein globales Allow-All",
  },
  denyContextInput: {
    ...EN.denyContextInput,
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "✓ {name} installiert - starte `deepmicode code` neu zur Verbindung (Neuladen fehlgeschlagen: {message})",
    restartBridge: "✓ {name} installiert - starte `deepmicode code` neu zur Verbindung",
  ...
  mcpBrowser: {
    ...
    title: "◈ MCP-Browser",
    empty:
      "Keine MCP-Server angehängt. Führe `deepmicode setup` aus, um welche auszuwählen, oder starte mit --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ führe `deepmicode setup` aus, um diesen Eintrag zu entfernen, oder behebe das zugrunde liegende Problem (fehlendes npm-Paket, Netzwerk usw.).",
    failedSetupConfigHint:
      "→ führe `deepmicode setup` aus, um fehlerhafte Einträge aus deiner gespeicherten Konfiguration zu entfernen.",
    abortedHint:
  ...
  checkpointPicker: {
    ...
    header: " ◈ DEEPMICODE · Checkpoint auswählen ",
  ...
  mcpCli: {
    ...
    morePagesAvailable: "▸ mehr Seiten verfügbar — `deepmicode mcp list --pages <n>` oder --all",
    ...
    moreLoaded:
      "… {count} weitere geladen — verwende `deepmicode mcp search <abfrage>` zum Filtern",
