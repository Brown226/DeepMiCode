...
export const EN: TranslationSchema = {
  ...
  cli: {
    description:
      "DeepMiCode - DeepSeek + Mimo AI coding agent, built for cache hits and cheap tokens.",
  ...
  ui: {
    welcome: "Run `deepmicode` any time to start chatting — your settings are remembered.",
  ...
  code: {
    workspaceConflict:
      "⚠ workspace contains another agent platform's files ({platforms}). DeepMiCode Code may read them as project content; relaunch with --dir <your-project> if that's not what you want.\n",
  ...
  slash: {
    ...
    hooks: {
      description:
        "list active hooks (settings.json under .deepmicode/) · reload re-reads from disk",
      argsHint: "[reload]",
    },
    permissions: {
      description:
  ...
  errors: {
    ...
    upstream5xxHead:
      "Upstream service unavailable ({status}) at {host} — the configured API endpoint returned a server error, not a DeepMiCode bug. Already retried 4× with backoff.",
  ...
  handlers: {
    basic: {
      ...
      aboutHeader: "DeepMiCode v{version} — a cache-first DeepSeek coding agent",
    ...
    admin: {
      ...
      updateNpxHint:
        "you're running via npx — the next `npx deepmicode ...` launch will auto-fetch.",
    ...
    edits: {
      undoCodeOnly:
        "/undo is only available inside `deepmicode code` — chat mode doesn't apply edits.",
      ...
      planCodeOnly:
        "/plan is only available inside `deepmicode code` — chat mode doesn't gate tool writes.",
      planOn:
      ...
      commitCodeOnly:
        "/commit is only available inside `deepmicode code` (needs a rooted git repo).",
      commitUsage:
      ...
      checkpointCodeOnly:
        "/checkpoint is only available inside `deepmicode code` — chat mode doesn't apply edits.",
      checkpointNone:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear are only available inside `deepmicode code` — they edit the project-scoped allowlist (`~/.deepmicode/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      noSession:
        "no session attached — `/plans` is per-session. Run `deepmicode code` in a project to get a session.",
      ...
      replayNoSession:
        "no session attached — `/replay` is per-session. Run `deepmicode code` in a project to get a session.",
      replayNoArchives:
    ...
    memory: {
      disabled:
        "memory is disabled (DEEPMICODE_MEMORY=off in env). Unset the var to re-enable — no DEEPMICODE.md or ~/.deepmicode/memory content will be pinned in the meantime.",
      noRoot:
        "no working directory on this session — `/memory` needs a root to resolve DEEPMICODE.md from. (Running in a test harness?)",
      listEmpty:
      ...
      layerGlobal: "  2. ~/.deepmicode/memory/global/ — your cross-project private memory.",
      layerProjectHash:
        "  3. ~/.deepmicode/memory/<project-hash>/ — this project's private memory.",
    ...
    mcp: {
      noServers:
        'no MCP servers attached. Run `deepmicode setup` to pick some, or launch with --mcp "<spec>". `deepmicode mcp list` shows the catalog. Note: model-invoked shell commands are gated per-call (allow once / allow always / deny) — no global allow-all flag.',
    ...
    init: {
      ...
      exists: "▸ DEEPMICODE.md already exists at {path}",
      ...
      info: "▸ /init — model will scan the project and synthesize DEEPMICODE.md.\n  The result lands as a pending edit; review with /apply or /walk.",
    },
    webSearchEngine: {
    ...
    skill: {
      ...
      listProjectScope:
        "  · <project>/.deepmicode/skills/<name>/SKILL.md  (or <name>.md)  — project scope",
      listGlobalScope: "  · ~/.deepmicode/skills/<name>/SKILL.md  (or <name>.md)  — global scope",
  ...
  pathConfirm: {
    ...
    allowAlwaysDesc:
      "remember `{prefix}` for this project (persisted in ~/.deepmicode/config.json)",
    deny: "deny",
  ...
  workspacePicker: {
    header: " ◈ DEEPMICODE · pick a workspace ",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ run `deepmicode setup` to remove this entry, or fix the underlying issue (missing npm package, network, etc.).",
    failedSetupConfigHint:
      "→ run `deepmicode setup` to remove broken entries from your saved config.",
    abortedHint:
