...
export async function spawnSubagent(opts: SpawnSubagentOptions): Promise<SubagentResult> {
  ...
  const taskPreview = opts.task.length > 30 ? `${opts.task.slice(0, 30)}â€¦` : opts.task;
  sink?.current?.({
    kind: "start",
    runId,
    task: taskPreview,
  ...
  const truncated =
    final.length > maxResultChars
      ? `${final.slice(0, maxResultChars)}\n\n[â€¦truncated ${final.length - maxResultChars} chars; ask the subagent for a tighter summary if you need more.]`
      : final;
