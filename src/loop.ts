...
export class CacheFirstLoop {
  ...
  constructor(opts: CacheFirstLoopOptions) {
    ...
    this.repair = new ToolCallRepair({
      ...
      isStormExempt,
      stormThreshold: parsePositiveIntEnv(process.env.DEEPMICODE_STORM_THRESHOLD),
      stormWindow: parsePositiveIntEnv(process.env.DEEPMICODE_STORM_WINDOW),
    });
  ...
  /** "New chat" — drops in-memory messages, archives the on-disk transcript so it survives in Sessions, keeps sessionName so the prefix cache stays warm. Re-runs the system-prompt builder if one was wired (issue #778: DEEPMICODE.md edits otherwise need a restart). */
