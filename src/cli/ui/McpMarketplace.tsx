/** `/mcp browse` modal 鈥?registry marketplace inside the chat session. */
...
export interface McpMarketplaceProps {
  ...
  /** Optional hot-reload 鈥?present in chat session, absent in standalone CLI use. */
  reloadMcp?: () => Promise<{
    added: string[];
    removed: string[];
...
interface State {
  ...
  selected: number;
  status: string;
  /** specs currently in config.mcp[] 鈥?refreshed after install/uninstall. */
  ...
  const fetchMore = useCallback(async () => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        loading: false,
        status: `+${r.newEntries} 路 ${state.handle?.cache.entries.length ?? 0} total${
          r.exhausted ? " 路 exhausted" : ""
        }`,
      }));
    } catch (err) {
  ...
  const doUninstall = useCallback(
    async (entry: RegistryEntry, installed: string) => {
      ...
      if (reloadMcp) {
        try {
          await reloadMcp();
          postInfo(`鉁?uninstalled ${entry.name} 鈥?bridge dropped`);
        } catch (err) {
          postInfo(
            `鉁?uninstalled ${entry.name} 鈥?restart \`deepmicode code\` to drop the bridge (reload failed: ${(err as Error).message})`,
          );
        }
      } else {
        postInfo(`鉁?uninstalled ${entry.name} 鈥?restart \`deepmicode code\` to drop the bridge`);
      }
    },
    [postInfo, reloadMcp],
  );
  ...
  const doInstall = useCallback(
    async (entry: RegistryEntry) => {
      ...
      if (!install) {
        setState((s) => ({
          ...s,
          status: `no install info for ${entry.name} 鈥?try \`npx -y @smithery/cli install ${entry.name}\``,
        }));
        return;
      }
      try {
        ...
        writeConfig({ ...cfg, mcp: next });
        setState((s) => ({ ...s, installedSpecs: next, status: `installed 鈫?${spec}` }));
        const envHint = install.requiredEnv?.length
          ? `  路  needs env: ${install.requiredEnv.join(", ")}`
          : "";
        if (reloadMcp) {
          try {
            ...
            if (failedHere) {
              postInfo(`鈻?installed ${entry.name} 鈥?bridge failed: ${failedHere.reason}${envHint}`);
            } else {
              postInfo(`鉁?installed ${entry.name} 鈥?bridged${envHint}`);
            }
          } catch (err) {
            postInfo(
              `鉁?installed ${entry.name} 鈥?restart \`deepmicode code\` to bridge (reload failed: ${(err as Error).message})${envHint}`,
            );
          }
        } else {
          postInfo(`鉁?installed ${entry.name} 鈥?restart \`deepmicode code\` to bridge${envHint}`);
        }
      } catch (err) {
      ...
      <Box marginTop={1} flexDirection="column">
        {window.length === 0 ? (
          ...
          window.map((e, i) => {
            ...
            const installedBadge = installedSpec ? " 鉁? : "";
            const pop = e.popularity !== undefined ? ` 路 ${e.popularity.toLocaleString()}` : "";
            return (
              <Box key={e.name}>
                <Text color={active ? COLOR.brand : undefined}>{active ? "鈻?" : "  "}</Text>
