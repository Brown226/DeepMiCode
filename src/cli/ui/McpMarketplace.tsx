    ...
    async (entry: RegistryEntry, installed: string) => {
      ...
      if (reloadMcp) {
        try {
          ...
          postInfo(
            `✓ uninstalled ${entry.name} — restart \`deepmicode code\` to drop the bridge (reload failed: ${(err as Error).message})`,
          );
        }
      } else {
        postInfo(`✓ uninstalled ${entry.name} — restart \`deepmicode code\` to drop the bridge`);
      }
    },
    [postInfo, reloadMcp],
  );
      ...
      try {
        ...
        if (reloadMcp) {
          try {
            ...
            postInfo(
              `✓ installed ${entry.name} — restart \`deepmicode code\` to bridge (reload failed: ${(err as Error).message})${envHint}`,
            );
          }
        } else {
          postInfo(`✓ installed ${entry.name} — restart \`deepmicode code\` to bridge${envHint}`);
        }
      } catch (err) {
