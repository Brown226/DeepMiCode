...
export const SLASH_COMMANDS: readonly SlashCommandSpec[] = [
  ...
  },
  {
    ...
    summary:
      "reasoning_effort cap 鈥?high is the safe default (vLLM/Azure compatible); max is a DeepSeek extension.",
  ...
  },
  {
    cmd: "about",
    group: "info",
    summary: "project info 鈥?version, website, repo, license",
  },

  { cmd: "sessions", group: "session", summary: "list saved sessions (current marked with 鈻?" },
  {
    cmd: "title",
    group: "session",
  ...
  { cmd: "mcp", group: "extend", summary: "list MCP servers + tools attached to this session" },
  {
    ...
    group: "extend",
    argsHint: "[uri]",
    summary: "browse + read MCP resources (no arg 鈫?list URIs; <uri> 鈫?fetch contents)",
  ...
  },
  {
    ...
    group: "extend",
    argsHint: "[name]",
    summary: "browse + fetch MCP prompts (no arg 鈫?list names; <name> 鈫?render prompt)",
  ...
  },
  {
    ...
    group: "code",
    argsHint: "[N|N,M|N-M]",
    summary: "drop pending edit blocks without writing (no arg 鈫?all; indices 鈫?that subset)",
    contextual: "code",
  },
  {
    cmd: "walk",
  ...
  },
  {
    ...
    summary:
      "edit-gate: review (queue) 路 auto (apply+undo) 路 yolo (apply+auto-shell) 路 plan (read-only). Shift+Tab cycles.",
    contextual: "code",
  ...
  },
  {
    ...
    group: "jobs",
    argsHint: "<id>",
    summary: "stop a background job by id (SIGTERM 鈫?SIGKILL after grace)",
    contextual: "code",
  },
  {
    cmd: "logs",
  ...
  },
  {
    ...
    summary:
      "switch web search backend 鈥?bing (default, works from CN without proxy), searxng (self-hosted), metaso (free 100/d), tavily (free 1000/mo), perplexity (AI-native), exa (AI-native), brave (independent index, free 2000/mo), or ollama (Ollama cloud web search). Provider with no key prompts inline config.",
  ...
  },
  {
    ...
    group: "advanced",
    argsHint: "[reload]",
    summary: "list active hooks (settings.json under .deepmicode/) 路 reload re-reads from disk",
  },
  {
    cmd: "permissions",
    group: "advanced",
...
export function suggestSlashCommands(
  ...
  const matches = SLASH_COMMANDS.filter((c) => {
    // Empty prefix = browsing the menu 鈥?show the full release command surface except
...
/** alias 鈫?canonical cmd map, derived from SLASH_COMMANDS at module init. */
