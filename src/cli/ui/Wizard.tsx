/** First-run / re-configure wizard 鈥?saves to `~/.deepmicode/config.json`. */
...
export function Wizard({
  ...
  const [previewTheme, setPreviewTheme] = useState<ThemeName>(() =>
    resolveThemePreference(initial?.theme ?? loadTheme(), process.env.DEEPMICODE_THEME),
  );

  const [step, setStep] = useState<Step>("language");
  const [data, setData] = useState<WizardData>(() => ({
    language: getLanguage(),
    theme: resolveThemePreference(initial?.theme ?? loadTheme(), process.env.DEEPMICODE_THEME),
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={theme.tone.brand} paddingX={1}>
      ...
      <Box marginTop={1} flexDirection="column">
        {THEME_NAMES.map((name, i) => (
          <Box key={name}>
            ...
            <Text color={theme.fg.meta}>{" 鈥?"}</Text>
      ...
      >
        ...
        <Box marginTop={1}>
          <Text color={theme.tone.accent}>{"鈼?"}</Text>
          <Text color={theme.tone.accent}>{t("wizard.themeSampleReasoning")}</Text>
        </Box>
        <Box>
          <Text color={theme.tone.info}>{"鈻?"}</Text>
        ...
        <Box marginTop={1}>
          ...
          <Text color={theme.fg.faint}>{" 路 "}</Text>
...
// (DeepSeek, DashScope/Tongyi, Moonshot, Zhipu, 鈥?, and 401/403 there
 ...
 * `--mcp "..."` wrapper 鈥?we store the inner spec directly.
 */
...
function quoteIfNeeded(s: string): string {
  // Escape backslashes BEFORE quotes 鈥?otherwise a trailing `\` in the
