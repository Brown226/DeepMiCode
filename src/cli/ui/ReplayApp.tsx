...
export function ReplayApp({ meta, pages }: ReplayAppProps) {
  ...
  // Start at the last page 鈥?more useful than "start from the beginning"
  ...
  const summary = {
    ...
    // Replay is read-only 鈥?no live last-turn prompt tokens to show.
    lastPromptTokens: 0,
    lastTurnCostUsd: 0,
  };
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginTop={1} paddingX={1}>
        <Box justifyContent="space-between">
          ...
          {meta ? (
            <Text dim>
              {meta.source}
              {meta.task ? ` 路 ${meta.task}` : ""}
              {meta.mode ? ` 路 ${meta.mode}` : ""}
            </Text>
          ) : null}
        </Box>
      ...
      <Box marginTop={1} paddingX={1} borderStyle="single" borderColor="ansi:blackBright">
        <Text dim>
          <Text bold>j</Text>/<Text bold>鈫?/Text>/<Text bold>space</Text> next 路 <Text bold>k</Text>
          /<Text bold>鈫?/Text> prev 路 <Text bold>g</Text> first 路 <Text bold>G</Text> last 路{" "}
