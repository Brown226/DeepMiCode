...
function editInExternal(initial: string): string | null {
  ...
  if (!editor) {
    process.stderr.write(
      "deepmicode commit: no $EDITOR / $VISUAL / $GIT_EDITOR set — can't open editor. Pick [a]ccept and `git commit --amend` afterwards.\n",
    );
    return null;
  }
  ...
  if (result.status !== 0) {
    ...
    process.stderr.write(
      `deepmicode commit: editor exited ${result.status} — keeping prior draft.\n`,
    );
    return null;
  }
  let edited: string;
...
export async function commitCommand(opts: CommitOptions = {}): Promise<void> {
  ...
  if (!diff) {
    process.stderr.write(
      "deepmicode commit: no staged changes and working tree is clean — nothing to commit.\n",
    );
    process.exit(1);
  }
  if (diff.source === "working-tree") {
    process.stderr.write(
      "deepmicode commit: nothing staged — drafting from working-tree diff. Stage your changes and re-run, or use the draft as a starting point.\n",
    );
  }
  if (diff.truncated) {
    process.stderr.write(
      "deepmicode commit: diff exceeded 80KB; head + tail sent to the model. Large diffs often produce vague drafts — consider committing in smaller chunks.\n",
    );
  }
  ...
  while (true) {
    ...
    try {
      ...
      process.stderr.write(`deepmicode commit: model call failed — ${(err as Error).message}\n`);
      process.exit(1);
    }
    if (!message) {
