/** `deepmicode mcp browse` 鈥?Ink TUI for the MCP marketplace. Lazy-loads pages on scroll. */
...
function McpBrowseApp() {
    ...
    (async () => {
      try {
        ...
        setState((s) => ({
          ...
          handle,
          loading: false,
          status: `${handle.source} 路 ${handle.cache.entries.length} entries${
            handle.fromCache ? ` 路 cached ${ageStr} ago` : ""
          }`,
        }));
      } catch (err) {
        if (cancelled) return;
  ...
  const fetchMore = useCallback(async () => {
    ...
    if (state.handle.cache.pagination.nextCursor === null) {
      setStatus("no more pages 鈥?registry exhausted");
      return;
    }
    setState((s) => ({ ...s, loading: true, status: "loading more鈥? }));
    try {
  ...
  return (
    <Box flexDirection="column" paddingX={1}>
      ...
      <Box marginTop={1} flexDirection="column">
        {window.length === 0 ? (
          <Text dim>{state.loading ? "loading鈥? : "no entries"}</Text>
        ) : (
          window.map((e, i) => {
            const idx = (start || 0) + i;
            ...
            return (
              <Box key={e.name}>
                <Text color={active ? "ansi:cyan" : undefined}>{active ? "鈻?" : "  "}</Text>
      ...
      {selected ? (
        <Box marginTop={1} flexDirection="column">
          ...
          {selected.install ? (
            <Text dim>
              {`spec: ${selected.install.runtime} ${selected.install.packageId ?? selected.install.url ?? "鈥?} 路 ${selected.install.transport}`}
            </Text>
          ) : (
            <Text dim>(smithery listing 鈥?install info not exposed)</Text>
          )}
      ...
      <Box marginTop={1}>
        <Text dim>type to filter 路 鈫戔啌 pick 路 enter install 路 tab load more 路 esc quit</Text>
      </Box>
    </Box>
  );
}
...
export interface McpBrowseOptions {
  /** Reserved 鈥?currently unused, kept for symmetry with other commands. */
  _unused?: never;
}
