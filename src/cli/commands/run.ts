...
export interface RunOptions {
  ...
  system: string;
  budgetUsd?: number;
  /** JSONL transcript path — lets `deepmicode replay` / `diff` audit this run. */
  transcript?: string;
...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (normalizedSpecs.length > 0) {
    ...
    for (const spec of normalizedSpecs) {
      ...
      try {
        ...
        // Non-fatal — skip and continue, same as `deepmicode chat`. A
  ...
  try {
    for await (const ev of loop.step(opts.task)) {
      ...
      if (ev.role === "assistant_final" && ev.stats?.usage) {
        // `deepmicode run` is often used in CI / scripting — we want
  ...
  if (opts.transcript) {
    ...
    process.stdout.write(`  → npx deepmicode replay ${opts.transcript}\n`);
  }
