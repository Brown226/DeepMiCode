...
const progressToStderr: FetchProgress = ({ source, page, entries }) => {
  if (page === 1 || page % 5 === 0) {
    process.stderr.write(`\r鈻?fetching ${source} registry 路 page ${page} 路 ${entries} entries`);
  }
};
...
export interface McpListOptions {
  json?: boolean;
  /** Skip network 鈥?only show the bundled MCP_CATALOG entries. */
  local?: boolean;
  /** Bypass cache TTL. */
  refresh?: boolean;
...
function printEntry(e: RegistryEntry, indent = "  "): void {
  ...
  const pop = e.popularity !== undefined ? ` 路 ${e.popularity.toLocaleString()} uses` : "";
  ...
  if (e.install?.requiredEnv?.length) {
    ...
    console.log(`${indent}    (smithery listing 鈥?install detail fetched lazily on install)`);
  }
}
...
export async function mcpListCommand(opts: McpListOptions = {}): Promise<void> {
  if (opts.local) {
    ...
    for (const entry of MCP_CATALOG) {
      ...
      if (entry.note) console.log(`               路 ${entry.note}`);
      console.log("");
    }
    return;
  }
...
export async function mcpInstallCommand(name: string, opts: McpInstallOptions = {}): Promise<void> {
  ...
  if (!entry.install && entry.source === "smithery") {
    process.stderr.write(`鈻?fetching smithery install detail for ${entry.name}鈥n`);
  ...
  if (entry.install.requiredEnv?.length) {
    ...
    console.log(
      `           (edit ${defaultConfigPath()} 鈥?values merge over process.env at spawn)`,
    );
  }
  console.log("");
  console.log(
    "Use it:  deepmicode chat   (or `deepmicode code`) 鈥?the server will be bridged automatically.",
  );
}
