...
MCP server (filesystem, github, slack, puppeteer, â€? while applying the
...
- **Surface tuning**: we only implement what DeepMiCode actually uses â€?  initialize + tools/list + tools/call. Resources, prompts, sampling,
...
```
src/mcp/
â”œâ”€â”€ types.ts      JSON-RPC 2.0 + MCP-specific message types
â”œâ”€â”€ stdio.ts      McpTransport interface + StdioTransport (spawn child)
â”œâ”€â”€ sse.ts        SseTransport (HTTP+SSE for remote/hosted servers)
â”œâ”€â”€ spec.ts       parseMcpSpec â€?parses --mcp CLI arg into transport-tagged spec
â”œâ”€â”€ catalog.ts    curated list of popular official MCP servers
â”œâ”€â”€ client.ts     McpClient: initialize / listTools / callTool
â”œâ”€â”€ registry.ts   bridgeMcpTools: MCP â†?ToolRegistry
â””â”€â”€ README.md     (this file)

tests/mcp.test.ts â€?in-process fake transport, no child processes
tests/mcp-sse.test.ts â€?in-process http.Server fake for SSE
```
...
| feature | status | note |
|---|---|---|
| CLI wiring (`deepmicode chat --mcp <cmd>`) | âœ?shipped | see Usage below |
| Bundled demo server | âœ?shipped | `examples/mcp-server-demo.ts`, exposes echo/add/get_time |
| Real-subprocess integration test | âœ?shipped | `tests/mcp-integration.test.ts` |
...
| SSE transport | âœ?shipped | `src/mcp/sse.ts` â€?pass `http(s)://â€¦` to `--mcp` |
| Streamable HTTP (2025-03-26 spec) | deferred | waiting for a real server to validate against |
| MCP server that DeepMiCode exposes | never | out of scope â€?DeepMiCode is a client |

## Usage (CLI)

`--mcp` is repeatable â€?attach one or many MCP servers; their tools become
...
// 3. Use them with the Cache-First Loop â€?same as any native tool
