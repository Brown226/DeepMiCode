...
deepmicode 可以把现有的 `chat`、`code` 或桌面端会话延伸到 QQ 上，作为远程通道使用。QQ 扩展的是当前会话，不是独立的新运行模式。
...
- 使用的是已经包含 QQ 支持的较新 deepmicode 版本
- QQ 账号已经完成实名认证
...
1. 启动 `deepmicode code` 或 `deepmicode chat`
2. 完成一次 QQ 连接
3. 从 QQ 发一条消息
4. 本地 deepmicode 会话继续运行
...
先确认本地 deepmicode 会话还在运行，而且 QQ 通道仍然在线：
