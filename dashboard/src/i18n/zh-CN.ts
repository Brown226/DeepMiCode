export const zhCN = {
  ...
  overview: {
    ...
    standaloneDesc:
      "只读磁盘视图。在 deepmicode code 内启动 /dashboard 以获取实时会话状态、MCP 和工具。",
    cockpit: "驾驶舱",
    balance: "余额",
    tokens7d: "tokens · 7 天",
    cacheHit: "缓存命中",
    ...
    budget: "预算",
    currentSession: "当前会话",
    noSession: "无活跃会话 — 在 deepmicode code 内执行 /dashboard 进行连接。",
    promptTok: "提示 tokens",
  ...
  usage: {
    ...
    noData: "暂无用量数据 — 在 deepmicode chat / code / run 中执行一轮，然后刷新。",
    windows: "滚动窗口",
    colWindow: "时间范围",
    colTurns: "轮次",
    colCacheHit: "缓存命中",
  ...
  sessions: {
    ...
    attachRequired:
      "实时会话操作需要已连接的 CLI 会话。请通过 deepmicode chat 启动，或在 TUI 会话中打开仪表盘。",
    activeChip: "当前",
    activePill: "当前",
  },
  tools: {
  ...
  permissions: {
    ...
    standaloneWarning:
      "修改操作需要在活跃的 deepmicode code 会话内执行 /dashboard — 独立模式的 deepmicode dashboard 无法确定要编辑哪个项目的允许列表。",
  },
  mcp: {
    loading: "加载 MCP…",
    ...
    saved: "已保存",
    savedRestart: "已保存 — 重启 deepmicode code 以桥接此服务器",
    ...
    whyUnbridged: "为什么未桥接？",
    whyUnbridgedDesc:
      "此规格存在于您的 config.json 中，但未桥接到实时会话。MCP 服务器在 deepmicode code 启动时连接；仪表盘本身无法生成子进程。",
    whyUnbridgedHint: "激活方法：重启 deepmicode code，然后刷新此仪表盘。",
    bridgeFailed: "桥接失败",
    bridgeFailedTitle: "桥接失败 · 在配置中",
    bridgeFailedHint:
      "已尝试桥接但失败。常见原因：URL 错误、需要鉴权、上游 404 / 5xx、本地命令缺失。修复后重启 deepmicode code 重试。",
  ...
  memory: {
    ...
    noFiles: "暂无记忆文件。",
    pickHint: "选择左侧的记忆文件。",
    pickDesc: "项目 DEEPMICODE.md 可提交；全局笔记存储在 ~/.deepmicode/memory/。",
    chars: "{count} 个字符",
    saved: "已保存 {scope}",
  ...
  hooks: {
    ...
    colScript: "脚本",
    noProject: "无活跃项目 — 在 deepmicode code 中打开 /dashboard 以编辑项目钩子。",
    saveReload: "保存并重载",
    discard: "放弃更改",
  ...
  skills: {
    ...
    builtinDesc:
      "内置技能随 DeepMiCode 一起发布；模型会自动识别。如需自定义，请创建同名的项目或全局技能。",
  ...
  system: {
    ...
    built: "● 已构建",
    none: "— 无",
    runIndex: "运行 deepmicode index 以构建",
    usageLog: "用量日志",
    backgroundJobs: "后台任务",
    noSession: "— 无会话",
    running: "{count} 个运行中",
  ...
  semantic: {
    ...
    installOllamaDesc: "DeepMiCode 不会为您运行包管理器。请先安装 Ollama，然后返回：",
    ...
    remoteProviderDesc:
      "在这里配置 OpenAI-Compatible embeddings 的完整 URL。DeepMiCode 会严格使用你提供的 URL 发起请求。",
    ollama: "Ollama",
    binary: "二进制",
    found: "已找到",
    missing: "缺失",
